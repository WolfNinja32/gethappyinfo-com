...
- **Stack invariants.** GitHub-direct repo (`WolfNinja32/gethappyinfo-com`), no Gitea mirror. Stdlib-only Python updater, no LLM, no Node, no deps. GH Actions pinned to `checkout@v6` + `setup-python@v6`.
...
- Do not add LLM curation, multi-source aggregation, analytics, email capture, or navigation to the site.
